...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Initializing The Greek Directory...');
    
    // Initialize Supabase
    supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
    console.log('✅ Supabase initialized');
    
    // Check for iOS web app
    if (isIOSWebApp()) {
        document.getElementById('refreshBtn').style.display = 'flex';
    }
    
    // Load starred listings
    loadStarredListings();
...
async function loadListings() {
    try {
        ...
        console.log('✅ Extracted subcategories:', Object.keys(SUBCATEGORIES).length, 'categories');
...
function renderListings() {
    ...
    if (currentView === 'grid') {
        ...
        container.innerHTML = displayedListings.map(l => {
            ...
            // Show chain info if applicable
            let chainBadge = '';
            if (l.is_chain && l.chain_name) {
                chainBadge = `<span class="badge" style="background: #6366f1; color: white;">📍 ${l.chain_name}</span>`;
            }
            ...
            return `
                <a href="${listingUrl}" class="bg-white rounded-lg shadow hover:shadow-lg transition-shadow overflow-hidden block relative">
                    ...
                    <div class="h-48 bg-gray-200 relative">
                        ...
                        ${badges.length > 0 || chainBadge ? `<div class="absolute top-2 left-2 flex gap-2 flex-wrap">${chainBadge}${badges.join('')}</div>` : ''}
                    </div>
                    <div class="p-4">
            ...
            allListings.forEach(listing => {
                ...
                if (state && state.includes(query) && !seen.has(`state-${state}`)) {
                    matches.push({ 
                        type: 'state', 
                        value: listing.state, 
                        display: US_STATES[listing.state] || listing.state 
                    });
                ...
                if (country.includes(query) && !seen.has(`country-${country}`)) {
                    matches.push({ 
                        ...
                        display: listing.country === 'USA' ? 'United States' : listing.country 
                    });
...
function setView(view) {
    ...
    document.getElementById('gridViewBtn').className = view === 'grid' ? 
        'p-2 rounded bg-white shadow-sm' : 'p-2 rounded';
    document.getElementById('listViewBtn').className = view === 'list' ? 
        'p-2 rounded bg-white shadow-sm' : 'p-2 rounded';
    document.getElementById('gridViewBtn2').className = view === 'grid' ? 
        'p-2 rounded bg-white shadow-sm' : 'p-2 rounded';
    document.getElementById('listViewBtn2').className = view === 'list' ? 
        'p-2 rounded bg-white shadow-sm' : 'p-2 rounded';
                ...
                <div class="map-popup">
                    ...
                    <div class="map-popup-content">
                        ...
                        <div class="map-popup-info">
                            ...
                            <a href="/listings/${categorySlug}/${listing.slug}" class="map-popup-title">${listing.business_name}</a>
...
function renderSplitViewListings() {
    // Similar to renderListings but for split view
...
function initSplitMap() {
    // Similar to initMap but for split view
...
function setupEventListeners() {
    ...
    ['openNowFilter', 'openNowFilter2', 'closedNowFilter', 'closedNowFilter2',
     'openingSoonFilter', 'openingSoonFilter2', 'closingSoonFilter', 'closingSoonFilter2',
     'hoursUnknownFilter', 'hoursUnknownFilter2', 'onlineOnlyFilter', 'onlineOnlyFilter2'].forEach(id => {
        document.getElementById(id).addEventListener('change', (e) => {
            const base = id.replace('2', '').replace('Filter', '');
            const filterVar = base.charAt(0).toLowerCase() + base.slice(1) + 'Only';
            window[filterVar] = e.target.checked;
            
            const id1 = base + 'Filter';
            const id2 = base + 'Filter2';
            document.getElementById(id1).checked = e.target.checked;
            document.getElementById(id2).checked = e.target.checked;
