const {
  SUPABASE_URL,
  SUPABASE_ANON_KEY,
  MAIN_CATEGORY_NAMES: MAIN_CATEGORIES
} = window.TGDCategoryMetadata;
const FORM_STORAGE_KEY = 'tgd_submit_form_draft_v4';

let SUBCATEGORY_MAP = window.TGDCategoryMetadata.createEmptySubcategoryMap();
...
async function loadDynamicSubcategories(){
  try {
    const metadata = await window.TGDCategoryMetadata.loadPublicCategoryMetadata({ client: supabaseClient });
    SUBCATEGORY_MAP = metadata.subcategoryMap;
  } catch (e) {
    console.warn('Could not load dynamic subcategories', e);
    SUBCATEGORY_MAP = window.TGDCategoryMetadata.createEmptySubcategoryMap();
  }
}
