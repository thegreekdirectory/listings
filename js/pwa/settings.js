// js/pwa/settings.js (WITH MAP APP SELECTION)
...
// ============================================
// PWA SETTINGS MANAGER - WITH MAP APP SELECTION
...
class SettingsManager {
    ...
    renderSettings() {
        ...
        container.innerHTML = `
            <div class="max-w-2xl mx-auto">
                ...
                <!-- Default Map App Section -->
                <div class="bg-white rounded-lg p-6 shadow-sm mb-6">
                    <h2 class="text-xl font-bold text-gray-900 mb-2">Default Map App</h2>
                    <p class="text-sm text-gray-600 mb-4">Select which map app to use when getting directions</p>
                    <div class="space-y-3">
                        <label class="flex items-center justify-between cursor-pointer p-3 rounded-lg hover:bg-gray-50 transition-colors border-2 ${defaultMapApp === '' ? 'border-blue-500 bg-blue-50' : 'border-transparent'}">
                            <span class="text-gray-700 font-medium">Ask Every Time</span>
                            <input type="radio" name="mapApp" value="" ${defaultMapApp === '' ? 'checked' : ''} class="w-5 h-5 text-blue-600">
                        </label>
                ...
                <div class="bg-white rounded-lg p-6 shadow-sm mb-6">
                    <h2 class="text-xl font-bold text-gray-900 mb-2">Dock Behavior</h2>
                    <p class="text-sm text-gray-600 mb-4">Control how the navigation dock behaves</p>
                    <label class="flex items-center justify-between cursor-pointer p-3 rounded-lg hover:bg-gray-50 transition-colors">
                        <div>
                            <div class="text-gray-900 font-medium">Auto-Hide Dock</div>
                            <div class="text-sm text-gray-600">Hide dock when scrolling down</div>
                        </div>
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('input[name="mapApp"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                const mapApp = e.target.value;
                localStorage.setItem('tgd_default_map_app', mapApp);
                
                document.querySelectorAll('input[name="mapApp"]').forEach(r => {
                ...
                if (window.PWAApp) {
                    window.PWAApp.showToast(
                        mapApp === '' ? 'Will ask which map app to use' :
                        mapApp === 'apple' ? 'Apple Maps set as default' :
        ...
        const autoHideToggle = document.getElementById('dockAutoHideToggle');
        if (autoHideToggle) {
            autoHideToggle.addEventListener('change', (e) => {
                const enabled = e.target.checked;
                if (this.dockManager) {
                    this.dockManager.setAutoHide(enabled);
                }
                if (window.PWAApp) {
                    window.PWAApp.showToast(enabled ? 'Auto-hide enabled' : 'Auto-hide disabled');
                }
            });
        }
