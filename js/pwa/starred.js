        ...
        return `
            <a href="${url}" class="bg-white rounded-lg shadow hover:shadow-lg transition-shadow overflow-hidden block">
                <div class="h-48 bg-gray-200 relative">
                    ${mainImage ? `<img src="${mainImage}" alt="${listing.business_name}" class="w-full h-full object-cover">` : '<div class="w-full h-full flex items-center justify-center text-gray-400">No image</div>'}
                    <button class="star-button starred" onclick="handleStarClick('${listing.id}', event)" data-listing-id="${listing.id}">
                ...
                <div class="p-4">
                    <div class="flex gap-3 mb-3">
                        ${listing.logo ? `<img src="${listing.logo}" alt="${listing.business_name} logo" class="w-16 h-16 rounded object-cover flex-shrink-0">` : '<div class="w-16 h-16 rounded bg-gray-200 flex-shrink-0"></div>'}
                        <div class="flex-1 min-w-0">
                            <span class="text-xs font-semibold px-2 py-1 rounded-full text-white block w-fit mb-2" style="background-color:#055193;">${listing.category}</span>
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => starredManager.init());
} else {
    starredManager.init();
}
