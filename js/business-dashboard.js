...
// Tagline hard limit accounting for meta suffix
...
let _currentMaxPhotos = 1;
let _currentMaxCtas   = 0;
    ...
    if (Array.isArray(dynSubs)) {
      dynSubs.forEach(row => {
        if (row.category && Array.isArray(row.subcategories)) {
          SUBCATEGORIES[row.category] = row.subcategories;
        }
      });
    }
...
function renderOverview() {
  ...
  const features = {
    ...
    FEATURED: [
      ...
      'Featured badge and enhanced placement',
      'Extended description (up to 2,000 characters)',
      'Photo gallery (up to 5 photos)',
      'Custom CTA button (1)',
      'Enhanced analytics — website, call & direction clicks'
    ],
    PREMIUM: [
      ...
      'Video embed (1 video)',
      'Custom CTA buttons (2)',
      'Comprehensive analytics including video plays'
    ]
  };

  const heroImg  = (listing.photos && listing.photos.length > 0) ? listing.photos[0] : listing.logo;
  const verified = listing.verified || listing.tier === 'FEATURED' || listing.tier === 'PREMIUM';
...
async function _loadOverviewStats() {
  ...
  try {
    ...
    // FREE only sees views + total engagement
      ...
      ${tier === 'FREE' ? `
        <div class="bp-upgrade-notice">
          ...
          <div>
            ...
            <p>Upgrade to Featured or Premium to see call clicks, website visits, direction requests, shares, and your recent event history.</p>
          </div>
        </div>
      ` : `
...
async function _fetchAnalyticsAggregates(listingId) {
  // Fetch all rows for this listing from listing_analytics and sum them up
  const { data, error } = await window.TGDAuth.supabaseClient
  ...
  (data || []).forEach(row => {
    ...
    // Count individual action rows too (event-level rows)
    if (row.action === 'view')       totals.views            += 1;
    if (row.action === 'call')       totals.call_clicks      += 1;
    if (row.action === 'website')    totals.website_clicks   += 1;
    if (row.action === 'directions') totals.direction_clicks += 1;
    if (row.action === 'share')      totals.share_clicks     += 1;
    if (row.action === 'video')      totals.video_plays      += 1;

    // Merge share platforms
...
function _buildAnalyticsCards(stats, tier) {
  ...
  let visible;
  if (tier === 'FREE')      visible = all.slice(0, 1).concat([{ label: 'Total Engagement', value: stats.call_clicks + stats.website_clicks + stats.direction_clicks + stats.share_clicks, grad: 'bg-grad-2', icon: STAR_SVG }]);
  else if (tier === 'FEATURED') visible = all.slice(0, 5);
  else                          visible = all;
...
function _buildEventLog(events) {
  ...
  const colors = {
    view: '#3b82f6', call: '#10b981', website: '#6366f1',
    directions: '#ef4444', share: '#f59e0b', video: '#0ea5e9'
  };

  const labels = {
    view: 'Page View', call: 'Phone Call', website: 'Website Visit',
    directions: 'Directions', share: 'Share', video: 'Video Play'
  };
...
function renderEditForm() {
  ...
  const maxPhotos  = tier === 'FREE' ? 1 : tier === 'FEATURED' ? 5 : 15;
  const maxCtas    = tier === 'PREMIUM' ? 2 : tier === 'FEATURED' ? 1 : 0;
  const hasVideo   = tier === 'PREMIUM';

  _currentMaxPhotos = maxPhotos;
  _currentMaxCtas   = maxCtas;
  ...
  document.getElementById('content-edit').innerHTML = `
    ...
    ${_section('media', IMAGE_SVG, 'Photos & Media', false, `
      ...
      <div class="bp-cf-config">
        ...
        <div class="bp-cf-config__fields" id="cfFields">
          ...
          <div class="bp-field col-span-2">
            <label class="bp-label" for="cfEndpoint">Upload Proxy Endpoint (recommended)</label>
            <input class="bp-input" type="url" id="cfEndpoint" placeholder="https://tgd-images-upload.thegreekdirectory.org">
            <span style="font-size:.75rem;color:var(--slate-400);">Use the proxy to avoid CORS issues.</span>
          </div>
        </div>
      </div>
    ...
    ${_section('info', EDIT_SVG, 'Additional Information', false, `
      <p style="font-size:.85rem;color:var(--slate-500);margin-bottom:16px;">
        Up to 5 custom label/value pairs shown on your listing page.
      </p>
      <div class="bp-form-grid">
        ${[0,1,2,3,4].map(i => {
    ...
    <!-- ── Custom CTAs ── -->
    ${maxCtas > 0 ? _section('ctas', STAR_SVG, `Custom CTA Buttons (${maxCtas} allowed)`, false, `
      <p style="font-size:.85rem;color:var(--slate-500);margin-bottom:16px;">
        Button name max 15 characters. Used for booking links, menus, order forms, etc.
      </p>
      ${[0,1].filter(i => i < maxCtas).map(i => {
...
window._toggleSubcat = function(sub) {
  ...
  if (idx > -1) {
    _selectedSubcats.splice(idx, 1);
    if (_primarySubcat === sub) _primarySubcat = _selectedSubcats[0] || null;
  } else {
    _selectedSubcats.push(sub);
    if (!_primarySubcat) _primarySubcat = sub;
  }
  _renderSubcats();
};

window._setPrimarySubcat = function(sub) {
  _primarySubcat = sub;
  _renderSubcats();
};
...
async function saveChanges() {
  ...
  if (tagline.length > taglineMax) return showToast(`Tagline must be ${taglineMax} characters or fewer for SEO compliance.`, 'error');
  ...
  // CTAs
  const customCtas = [];
  for (let i = 0; i < 2 && i < _currentMaxCtas; i++) {
  ...
  const mergedPhotos   = [...existingPhotos, ..._uploadedImages.photos].slice(0, _currentMaxPhotos);
  const updatedLogo    = _uploadedImages.logo    || listing.logo    || null;
  const updatedVideo   = _uploadedImages.video   || listing.video   || null;
  ...
  const updates = {
    ...
    reviews: {
      // Preserve locked fields; allow adding if empty
...
async function _performSave(updates) {
  ...
  try {
    ...
    // 2. Try to trigger static page regeneration via edge function (best-effort)
    try {
      ...
      if (session) {
        ...
        showToast('Live listing page regenerated.', 'success', 3000);
      }
    } catch (edgErr) {
      // Non-fatal: data is saved, page will regenerate next time admin saves
      console.warn('Edge function unavailable; page will regenerate on next admin save.', edgErr);
    }
  ...
  try {
    ...
    const save = () => {
      localStorage.setItem(CF_STORAGE_KEY, JSON.stringify({
        ...
        uploadEndpoint: document.getElementById('cfEndpoint')?.value.trim()
      }));
    };
    ['cfAccountId','cfApiKey','cfEndpoint'].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      const key = id === 'cfAccountId' ? 'accountId' : id === 'cfApiKey' ? 'apiKey' : 'uploadEndpoint';
      el.value = cfg[key] || '';
...
async function _uploadToCloudflare(file) {
  const { uploadEndpoint, accountId, apiKey } = _cfConfig();
  const endpoint = uploadEndpoint || UPLOAD_PROXY;
  ...
  if (endpoint) {
    ...
    url = (variants[0] || json?.result?.url || '').replace('https://imagedelivery.net', 'https://images.thegreekdirectory.org');
    ...
    if (!res.ok || !json.success) throw new Error(json?.errors?.[0]?.message || 'Upload failed');
    const variants = json?.result?.variants || [];
    url = (variants[0] || '').replace('https://imagedelivery.net', 'https://images.thegreekdirectory.org');
  } else {
    throw new Error('No upload endpoint or Cloudflare credentials configured.');
  }
...
window._handleLogoUpload = async function(event) {
  ...
  try {
    const url = await _uploadToCloudflare(file);
    ...
    _setUploadStatus(`Logo upload failed: ${err.message}`, 'error');
  }
};
...
window._handlePhotosUpload = async function(event) {
  ...
  const newUploads = _uploadedImages.photos.length;
  const slots     = _currentMaxPhotos - existing.length - newUploads;

  if (slots <= 0) {
    showToast(`Photo limit reached (${_currentMaxPhotos} max for your plan).`, 'warning');
    return;
  }
  ...
  for (let i = 0; i < toUpload.length; i++) {
    try {
      const url = await _uploadToCloudflare(toUpload[i]);
      _uploadedImages.photos.push(url);
      if (preview) {
        const idx = existing.length + _uploadedImages.photos.length - 1;
        preview.insertAdjacentHTML('beforeend', _photoThumb(url, idx, 'new'));
      }
      successCount++;
    } catch (err) {
      _setUploadStatus(`Photo ${i+1} failed: ${err.message}`, 'error');
    }
  }

  if (successCount > 0) {
    _setUploadStatus(`${successCount} photo(s) uploaded successfully.`, 'success');
  }
};
...
window._handleVideoUpload = async function(event) {
  ...
  try {
    ...
    _setUploadStatus(`Video upload failed: ${err.message}`, 'error');
  }
};

window._removePhoto = function(url, type) {
  if (type === 'existing') {
    _removedPhotos.push(url);
    window.BP.currentListing.photos = (window.BP.currentListing.photos || []).filter(u => u !== url);
  } else {
    _uploadedImages.photos = _uploadedImages.photos.filter(u => u !== url);
  }
  // Re-render photo grid
  const preview = document.getElementById('photosPreview');
  if (preview) {
    const all = [
      ...(window.BP.currentListing?.photos || []).filter(u => !_removedPhotos.includes(u)),
      ..._uploadedImages.photos
    ];
...
function renderSettings() {
  ...
  _settingsVis = {
    ...
    email:     !!owner.email_visible,
    phone:     !!owner.phone_visible
  };
  ...
  document.getElementById('content-settings').innerHTML = `
    <div class="bp-page-header">
      <div>
        <h1>Settings</h1>
        <p>Manage your contact visibility and account security.</p>
      </div>
    </div>
      ...
      <div class="bp-card-body">
        ...
        <div class="bp-vis-row">
          <div>
            ...
            <div class="bp-vis-row__sub">
              <div id="settingsPhoneWrap" style="margin-top:6px;max-width:360px;"></div>
            </div>
          </div>
          <label class="bp-toggle-switch">
...
window._onVisChange = function(field, checked) {
  if (field === 'nameTitle' && !checked) {
    // Turning off name also turns off email + phone visibility
    _settingsVis.nameTitle = false;
    _settingsVis.email     = false;
    _settingsVis.phone     = false;
    ...
    // Cannot show email/phone without name
