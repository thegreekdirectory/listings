...
export const EVENTS_APP_SHELL_HTML = `    <!-- TOOLBAR — filters trigger, view toggle (grid / list / calendar / map), sort.
         Restructured to match listings.html's own #mobileControls pattern:
         a single row (not two separately-wrapping flex groups) with
         Filters first, then the view toggle immediately to its right —
         "the view options... should be to the right of the Filters
         button when on a mobile(-like) viewport." #eventResultsCount
         removed from this row entirely: listings.html has no results
         count in its own #mobileControls at all (its one and only
         #resultsCount lives in the desktop-only #normalViewControls,
         which #eventDesktopFiltersRow below already mirrors) — showing
         one here would be adding something Listings doesn't have in
         this position, not matching it. Sort and Subscribe keep their
         place after the view toggle since Listings has no equivalent
         controls to pattern-match against for those two specifically
         (its own mobile row is Filters/Hide-Starred/Map/Refresh, none of
         which are one-to-one events equivalents); this file already
         diverges from Listings there by necessity, not by choice. -->
    <div class="max-w-7xl mx-auto px-4">
        <div class="flex items-center gap-2 pb-3 border-b border-gray-200 events-toolbar-row">
            ...
            <div class="add-to-calendar-wrap">
                ...
                <div class="add-to-calendar-menu hidden" id="feedSubscribeMenu">
                    ...
                    <button type="button" class="event-no-bounce" onclick="copyFeedLink()">Copy feed link</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Overlay slot the filter panel moves into for Calendar/Map (see
         the GRID / LIST VIEW comment further below for the full
         reasoning). Placed here — immediately after the panel's own
         static position and before Map/Calendar — specifically so the
         panel renders ABOVE those views once it moves here, not below
         them: later DOM siblings paint below earlier ones in normal
         document flow, so this slot's position in the DOM is what
         actually determines stacking order here, not any z-index. -->
    ...
    <!-- GRID / LIST VIEW — wrapped in .desktop-layout/.desktop-filters/
         ...
         #eventFilterPanelOverlaySlot (now placed right after the static
         #eventFilterPanel block above, i.e. before Map/Calendar in the
         DOM — moved there from originally sitting AFTER both, which was
         the confirmed cause of a reported bug: the panel rendered below
         Map/Calendar instead of above them, since later DOM siblings
         paint below earlier ones in normal flow) whenever Calendar or Map
         is the active view, so the panel becomes a normal slide-down/
         overlay there instead, per "default is Side View, but when it's
         in Calendar or Map it toggles on top." One panel, moved between
         two slots — not a second duplicated filter form — since Listings'
         own actual second-form approach (#desktopFilterPanel, with a
         fully parallel set of "2"-suffixed ids) is unused dead markup in
         that codebase already (no JS references it), not a pattern worth
    ...
    <div class="max-w-7xl mx-auto px-4 py-4">
        <div class="desktop-layout" id="eventDesktopLayout">
            ...
            <div class="desktop-content">
                <!-- Desktop-width "# events found" + Filters-toggle pairing —
                     mirrors listings.html's own #resultsCount +
                     desktopFilterToggleBtn placement inside .desktop-content
                     exactly (to the left of the toggle button, both sitting
                     in the content column rather than the main toolbar).
                     Only relevant at >=1024px, where the sidebar concept
                     exists at all — hidden below that via
                     .events-desktop-filters-row (see css/events.css),
                     since the main toolbar's #eventFilterBtn already
                     covers Filters at narrower widths where there's no
                     sidebar to show/hide. Shown only while the sidebar is
                     HIDDEN (sidebarFiltersVisible === false) — once shown,
                     the sidebar's own "Hide filters" control (inside
                     #eventFilterPanel's header row) takes over, matching
                     Listings' desktopFilterToggleBtn only appearing in the
                     "sidebar not active" branch of checkFilterPosition(). -->
                <div class="events-desktop-filters-row flex items-center justify-between mb-3" id="eventDesktopFiltersRow">
                    <p class="text-sm text-gray-600" id="eventDesktopResultsCount"></p>
                    <button id="eventShowFiltersBtn" class="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-300 rounded-lg text-sm font-medium shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="#045093" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4h18M6 8h12M10 12h4"/></svg>
                        Filters
                    </button>
                </div>
