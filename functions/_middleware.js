export async function onRequest(context) {
  ...
  const hasCookie = cookieHeader.includes(`${COOKIE_NAME}=${SECRET_VALUE}`);
  const hasQuery = url.searchParams.get(SECRET_PARAM) === SECRET_VALUE;

  // 1. Check if the current request has the secret OR the cookie
  ...
  // 3. If it's NOT HTML, just send it (with the cookie attached to persist access)
  const contentType = response.headers.get("content-type") || "";
  if (!contentType.includes("text/html")) {
    const nonHtmlResponse = new Response(response.body, response);
    nonHtmlResponse.headers.append("Set-Cookie", `${COOKIE_NAME}=${SECRET_VALUE}; Path=/; Max-Age=31536000; SameSite=Lax`);
  ...
  // Set the access cookie on the final HTML response so they don't have to use the parameter next time
  const finalResponse = new Response(htmlResponse.body, htmlResponse);
  finalResponse.headers.append("Set-Cookie", `${COOKIE_NAME}=${SECRET_VALUE}; Path=/; Max-Age=31536000; SameSite=Lax`);
